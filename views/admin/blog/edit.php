<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h2 class="h3 font-monospace text-light m-0">Edit Blog Article</h2>
        <p class="text-secondary small m-0">Modify content, metadata, and styling with the Rich Text Editor</p>
    </div>
...
<form action="<?= Url::to('/admin/blog/' . $post['id']) ?>" method="POST" id="blogForm">
        ...
        <div class="col-lg-8">
            <div class="card-custom p-4 mb-4">
                <div class="mb-3">
                    ...
                    <input type="text" name="title" class="form-control form-control-lg font-heading fw-bold" value="<?= Security::e($post['title']) ?>" required>
                </div>

                <div class="mb-3">
                    <label class="form-label text-warning font-monospace small">Short Excerpt / Summary</label>
                    <textarea name="summary" class="form-control" rows="2"><?= Security::e($post['summary'] ?? '') ?></textarea>
                </div>
        ...
        <div class="col-lg-4">
            <div class="card-custom p-4 mb-4">
                ...
                <div class="mb-3">
                    ...
                    <select name="category_id" class="form-select font-monospace">
                        <?php foreach ($categories as $cat): ?>
                            <option value="<?= $cat['id'] ?>" <?= (int)$post['category_id'] === (int)$cat['id'] ? 'selected' : '' ?>><?= Security::e($cat['name']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="url" name="featured_image" class="form-control font-monospace small" value="<?= Security::e($post['featured_image']) ?>">
                </div>

                <div class="form-check mb-3">
                    <input class="form-check-input" type="checkbox" name="is_featured" id="isFeaturedCheck" value="1" <?= !empty($post['is_featured']) ? 'checked' : '' ?>>
                ...
                <div class="form-check mb-4">
                    <input class="form-check-input" type="checkbox" name="is_sticky" id="isStickyCheck" value="1" <?= !empty($post['is_sticky']) ? 'checked' : '' ?>>
                ...
                <button type="submit" class="btn btn-gold w-100 py-3 font-monospace fw-bold">
                    <i class="bi bi-save-fill me-1"></i> Update & Save Changes
                </button>
            </div>
        </div>
    </div>
